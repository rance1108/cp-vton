...
def train_gmm(opt, train_loader, G_A, G_B, D_A, D_B, board):
    ...
    criterionRelight = m_shared.cuda()
    ...
    for step in range(opt.keep_step + opt.decay_step):
        ...
        if (step+1) % opt.display_count == 0:
            ...
            print('step: %8d, time: %.3f, loss_G_A: %4f, loss_L1_B: %4f , loss_vgg_B: %4f ' \
                % (step+1, t, loss_L1.item(), loss_L1_B.item(), loss_vgg_B.item()), flush=True)
